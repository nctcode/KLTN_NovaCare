...
export function AdminSidebar() {
  ...
  return (
    <aside
      className={`bg-slate-950 text-white border-r border-slate-800 transition-all duration-300 flex flex-col z-30 shrink-0 ${
        collapsed ? 'w-20' : 'w-64'
      }`}
    >
      {/* Sidebar Header Logo */}
      <div className="h-16 border-b border-slate-800/80 flex items-center justify-between px-4">
        <Link href="/admin" className="flex items-center gap-3 overflow-hidden">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#0c4b39] to-emerald-600 flex items-center justify-center font-black text-xl text-[#66FF33] shadow-md shrink-0">
            N
          </div>
          {!collapsed && (
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-slate-400 font-medium">Hệ thống Quản trị</span>
            </div>
          )}
        </Link>
      ...
      <nav className="flex-1 p-3 space-y-1.5 overflow-y-auto">
        {ADMIN_MENU_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-[#66FF33]' : 'text-slate-400'}`} />
      ...
      <div className="p-3 border-t border-slate-800">
        <Link
          href="/"
          target="_blank"
          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-bold transition border border-slate-800"
