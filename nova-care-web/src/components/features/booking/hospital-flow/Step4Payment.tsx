...
export function Step4Payment({
  ...
  const handleProcessBooking = async () => {
    ...
    try {
      ...
      // 2. Confirm appointment for AT_HOSPITAL or QR_CODE payments
      if ((paymentMethod === 'AT_HOSPITAL' || paymentMethod === 'QR_CODE') && appointment?.id) {
        try {
          await appointmentService.confirm(appointment.id);
        } catch (confirmErr) {
          console.warn('Confirm appointment warning:', confirmErr);
        }
      } else if (paymentMethod === 'VNPAY' && appointment?.id) {
        try {
